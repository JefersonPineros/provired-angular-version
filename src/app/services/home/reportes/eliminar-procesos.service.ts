...
export class EliminarProcesosService {
  ...
  public getListEliminarProcesos(
    ...
    return this.http
      .post(environment.apiBaseUrl + 'eliminacionMasiva', filter)
      .pipe(
        map((res: any) => {
  ...
  public deleteProcesos(data: CaptchaSendDataModel): Observable<any> {
    ...
    return this.http.delete(
      environment.apiBaseUrl + 'eliminacionMasiva/delete',
      httpOptions
    );
  }
  ...
  public transferirProcesos(data: CaptchaSendDataModel): Observable<any> {
    return this.http.post(
      `${environment.apiBaseUrl}eliminacionMasiva/traspaso`,
      data
    );
  }

  public getUsersTransfer(userId: number): Observable<any> {
    return this.http.get(`${environment.apiBaseUrl}user/child/${userId}`).pipe(
      map((res: any) => {
        let status = res['status' as any];
        if (status == 200) {
          return res.data;
