...
export class ProcesosGeneralesComponent implements OnInit {
  ...
  servicePage(page: any) {
    ...
    this.filterGeneral.etiqueta = this.filterGeneral.etiqueta
      ? this.filterGeneral.etiqueta
      : '';
    this.filterGeneral.radicacion = this.filterGeneral.radicacion
      ? this.filterGeneral.radicacion
      : '';
    this.filterGeneral.demandante_demandado = this.filterGeneral
      .demandante_demandado
      ? this.filterGeneral.demandante_demandado
      : '';
    this.filterGeneral.from = page.first;
    this.filterGeneral.rows = page.rows;

    this.procesosGeneralesService
      .getListProcesos(this.filterGeneral)
      .subscribe({
        next: (res) => {
  ...
  downloadReport() {
    ...
    this.procesosGeneralesService
      .getReportListProcesos(this.filterProceso)
      .subscribe({
        next: (res) => {
          if (res.status == 200) {
